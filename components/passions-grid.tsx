...
const hobbies = [
  {
    ...
    illustration: "/hobbies/fitness.png",
  },
  {
    id: "reading",
    icon: BookOpen,
    title: "Reading",
    description: "Exploring new ideas through books and articles",
    illustration: "/hobbies/reading-chair.png",
  },
  {
    id: "meditation",
    icon: Sparkles,
    title: "Meditation",
    description: "Finding balance and inner peace through mindfulness",
    illustration: "/hobbies/meditation.png",
  },
  {
    id: "coding",
    icon: Code,
    title: "Coding",
    description: "Building creative solutions and learning new technologies",
    illustration: "/hobbies/coding.png",
  },
  {
    id: "coffee",
    icon: Coffee,
    title: "Coffee",
    description: "Appreciating the art and science of great coffee",
    illustration: "/hobbies/reading.png",
  },
  {
    id: "music",
    icon: Music,
    title: "Music",
    description: "Finding inspiration through diverse sounds and rhythms",
    illustration: "/hobbies/reading.png",
  },
];
...
export function PassionsGrid() {
    ...
    const animate = () => {
      setBubbles((prev) =>
        prev.map((b) => {
          ...
          // --- Customizable Bounce Area ---
          const topPadding = -100;   // allow bubbles to float into heading
          ...
          // Right bounce
          if (x >= containerWidth - size - rightPadding) {
            x = containerWidth - size - rightPadding;
            vx = -Math.abs(vx);
          }
          // Top bounce
          if (y <= topPadding) {
          ...
          // Bottom bounce
          if (y >= containerHeight - size - bottomPadding) {
            y = containerHeight - size - bottomPadding;
            vy = -Math.abs(vy);
          }
  ...
  // Auto cycle central illustration
  useEffect(() => {
    if (!hoveredHobby) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative min-h-screen bg-about-bg py-12 px-4 sm:py-16 md:py-20 lg:px-8 overflow-hidden"
    >
          ...
          return (
            <div
              ...
              className={`absolute transition-all duration-500 ${
                isActive ? "scale-150 opacity-100" : "scale-90 opacity-0"
              }`}
            >
              <img
          ...
          >
            <div
              className={`group relative flex h-full w-full items-center justify-center rounded-full border-2 border-accent-green/20 bg-white/80 backdrop-blur-sm transition-all duration-500 hover:scale-110 hover:border-accent-green hover:bg-white hover:shadow-2xl hover:shadow-accent-green/20 ${
                hoveredHobby === b.id
                  ? "scale-110 border-accent-green bg-white shadow-2xl"
                  : ""
              }`}
            >
              ...
              >
                <h3 className="mb-1 sm:mb-2 font-bold text-foreground text-base sm:text-lg">
                  {hobby.title}
                </h3>
